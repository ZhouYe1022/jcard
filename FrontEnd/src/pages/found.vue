<template>
  <div class="page-found">
    ...
    <form class="transparent-wrapper" v-if="currentStage === 'contact-info-form'" @submit.prevent="createNewRecord">
      <h1>捡到一张卡</h1>
      <p>失主还未在系统上录入信息<br>姓名 <b>{{userInput.cardName}}</b>&nbsp;&nbsp;&nbsp;学号 <b>{{userInput.cardNumber}}</b></p>
      <textarea v-model="userInput.message" rows="9" class="form-control" placeholder="请在此输入你的联系方式，以便丢卡的小伙伴联系你" required></textarea>
      <div class="btn-container">
        <button class="btn btn-white" type="submit">提交</button>
        <button type="button" class="btn btn-white" @click="backToIndex">取消</button>
      </div>
...
export default {
  data() {
    ...
    methods: {
      ...
      createNewRecord() {
        this.$http.post('/api/found', {
          OwnerName: this.userInput.cardName,
          CardNumber: this.userInput.cardNumber,
          Message: this.userInput.message,
          FounderName: 'unknown',
        }).then((res) => {
          window.sweetAlert('成功', '已经添加记录，等待失主联系你吧');
          this.backToIndex();
        }, (e) => {
          window.sweetAlert('请求出错了', e.data || e, 'error');
        });
      ...
      resolveRecord() {
        this.$http.put('/api/lost/' + this.result.id, {
          Status: 1,
          CardNumber: this.userInput.cardNumber
        }).then((res) => {
